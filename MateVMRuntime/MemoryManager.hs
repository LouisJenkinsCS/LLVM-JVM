module MateVMRuntime.MemoryManager   
    ( AllocationManager(..)
    , buildGCAction
    , dereference
    , RootSet
...
import MateVMRuntime.GC hiding (size)
import qualified MateVMRuntime.StackTrace as T
import qualified MateVMRuntime.JavaObjectsGC as GCObj
import MateVMRuntime.JavaObjectsGC(hasMTable) -- only instances for Ptr a
